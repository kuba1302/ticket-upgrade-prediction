...
class Pipeline:
    def __init__(self, data_path: str = Path(os.getcwd()) / "WEC2022_Data", data_type: str = 'train', model_type: str = 'predict_upgrade'):
        self.data_path, self.data_type, self.model_type = data_path, data_type, model_type
        self.df = self.merge_files()

    def read_csv_file(self, file_prefix) -> pd.DataFrame:
        return pd.read_csv(self.data_path / f'{file_prefix}_{self.data_type}.csv', sep=';')

    def read_all_files(self) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
        bkg = self.read_csv_file('BKG').drop(columns=self.get_bkg_drop_cols()) if self.data_type == 'train' else self.read_csv_file('BKG')
        tkt = self.read_csv_file('TKT')
        fcp = self.read_csv_file('FCP')
        return bkg, tkt, fcp[fcp.UPGRADED_FLAG == 'Y'] if self.model_type == 'predict_when_upgrade' else fcp

    def merge_files(self) -> pd.DataFrame:
        bkg, tkt, fcp = self.read_all_files()
        return fcp.merge(tkt, on='TICKET_NUMBER', how='left').merge(bkg, on='BOOKING_ID', how='left')
    ...
    def get_cols_to_drop_for_training(self) -> list:
        cols = ['UPGRADE_TYPE']
        cols.append('UPGRADE_SALES_DATE') if self.model_type == 'predict_upgrade' else cols.append('UPGRADED_FLAG')
        return cols

    def get_new_cols_for_predict_when_model(self) -> None:
        self.df["purchase_time_diff"] = self.df["UPGRADE_SALES_DATE"] - self.df["BOOKING_DEPARTURE_TIME_UTC"]
        self.df["purchase_time_diff"] = self.df["purchase_time_diff"].apply(lambda x: x.days)
    ...
    def filter_wrong_ticket_prices(self) -> None:
        self.df[self.df['TOTAL_PRICE_PLN'] > 0]

    @staticmethod
    def get_datetime_columns() -> list:
        return ["UPGRADE_SALES_DATE", "BOOKING_DEPARTURE_TIME_UTC", "FLIGHT_DATE_LOCAL", "SALES_DATE", "BOOKING_ARRIVAL_TIME_UTC",
                "TIME_DEPARTURE_LOCAL_TIME"]

    def convert_datetime_columns_to_pandas_format(self) -> None:
        self.df[self.get_datetime_columns()] = self.df[self.get_datetime_columns()].apply(pd.to_datetime)

    def filter_wrong_booking_window(self) -> None:
        self.df[self.df['BOOKING_WINDOW_D'] != -1]

    def filter_nonpositive_flight_distance(self) -> None:
        self.df[self.df['FLIGHT_DISTANCE'] >= 0]

    def get_sale_to_flight_time(self) -> None:
        self.df["sale_to_flight_time"] = self.df["FLIGHT_DATE_LOCAL"] - self.df["SALES_DATE"]
        self.df["sale_to_flight_time"] = self.df["sale_to_flight_time"].apply(lambda x: x.days)
    ...
    def map_stay_length(self) -> None:
        self.df["STAY_LENGTH_D"] = self.df["STAY_LENGTH_D"].replace(self.get_stay_length_map())

    def get_flight_len(self) -> None:
        self.df["flight_len"] = self.df["BOOKING_ARRIVAL_TIME_UTC"] - self.df["BOOKING_DEPARTURE_TIME_UTC"]
        self.df["flight_len"] = self.df["flight_len"].apply(lambda x: x.seconds / 3600)

    def map_departure_time_to_hours(self) -> None:
        self.df["TIME_DEPARTURE_LOCAL_TIME"] = self.df["TIME_DEPARTURE_LOCAL_TIME"].apply(
            lambda x: x.hour
        )
    ...
    def check_for_add_upgrade(self) -> None:
        ...
        self.df['if_additional_upgrade'] = np.where(
            np.isin(self.df["TICKET_NUMBER"], emd["REFERENCE_TICKET_NUMBER"].unique()), 1, 0
        )
    ...
    def check_for_sus_aircraft(self) -> None:
        self.df["is_sus_aircraft"] = np.where(np.isin(self.df["AIRCRAFT_TYPE"], self.get_sus_air_type()), 1, 0)

    def map_target_variable_for_training(self) -> None:
        self.df["UPGRADED_FLAG"] = self.df["UPGRADED_FLAG"].map(self.get_yes_no_binary_map())

    def check_for_sus_payment(self) -> None:
        self.df["is_sus_payment"] = np.where(np.isin(self.df["FORM_OF_PAYMENT"], self.get_sus_payment()), 1, 0)

    def get_intinerary_len(self) -> None:
        self.df["intinerary_len"] = self.df["INTINERARY"].apply(lambda x: len(x.split("-")))

    def check_for_sus_currency(self) -> None:
        self.df["is_sus_currency"] = np.where(np.isin(self.df["FORM_OF_PAYMENT"], self.get_sus_currency()), 1, 0)
    ...
    def map_corporate_contract_flg(self) -> None:
        self.df["CORPORATE_CONTRACT_FLG"] = self.df["CORPORATE_CONTRACT_FLG"].map(self.get_yes_no_binary_map())

    def map_loyal_customer(self) -> None:
        self.df["LOYAL_CUSTOMER"] = self.df["LOYAL_CUSTOMER"].map(self.get_yes_no_binary_map())

    def map_booking_long_houl_flag(self) -> None:
        self.df["BOOKING_LONG_HOUL_FLAG"] = self.df["BOOKING_LONG_HOUL_FLAG"].map(self.get_yes_no_binary_map())

    def map_booking_domestic_flag(self) -> None:
        self.df["BOOKING_DOMESTIC_FLAG"] = self.df["BOOKING_DOMESTIC_FLAG"].map(self.get_yes_no_binary_map())
    ...
    def clean_df(self):
        ...
        self.df = self.df.drop(columns=self.get_cols_to_drop_for_training()) if self.data_type == 'train' else self.df
    ...
    def get_oh_encoding(self) -> None:
        return pd.get_dummies(self.df[self.get_oh_cols()], prefix=self.df.get_oh_cols(), drop_first=True)

    def __call__(self) -> None:
        self.clean_df()
        return pd.concat([self.df, self.get_oh_encoding], axis=1).drop(columns=self.get_oh_cols())
