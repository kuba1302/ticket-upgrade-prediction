...
def clean_df(data, data_path, data_type="train", model_type="predict_upgrade"):
    ...
    if model_type == "predict_upgrade":
        ...
        df["BOOKING_DEPARTURE_TIME_UTC"] = pd.to_datetime(
            df["BOOKING_DEPARTURE_TIME_UTC"]
        )

        df["purchase_time_diff"] = (
            df["UPGRADE_SALES_DATE"] - df["BOOKING_DEPARTURE_TIME_UTC"]
        )
        df["purchase_time_diff"] = df["purchase_time_diff"].apply(
            lambda x: x.days
        )
    ...
    df["sale_to_flight_time"] = df["sale_to_flight_time"].apply(
        lambda x: x.days
    )

    stay_lenght_map = {-9999: np.nan}
    df["STAY_LENGTH_D"] = df["STAY_LENGTH_D"].replace(stay_lenght_map)
    df["BOOKING_ARRIVAL_TIME_UTC"] = pd.to_datetime(
        df["BOOKING_ARRIVAL_TIME_UTC"]
    )
    df["BOOKING_DEPARTURE_TIME_UTC"] = pd.to_datetime(
        df["BOOKING_DEPARTURE_TIME_UTC"]
    )
    ...
    df["flight_len"] = (
        df["BOOKING_ARRIVAL_TIME_UTC"] - df["BOOKING_DEPARTURE_TIME_UTC"]
    )
    df["flight_len"] = df["flight_len"].apply(lambda x: x.seconds / 3600)

    df["TIME_DEPARTURE_LOCAL_TIME"] = pd.to_datetime(
        df["TIME_DEPARTURE_LOCAL_TIME"]
    )
    ...
    def get_if_add_upgrade(df, data_type):
        ...
        return np.where(
            np.isin(
                df["TICKET_NUMBER"], emd["REFERENCE_TICKET_NUMBER"].unique()
            ),
            1,
            0,
        )
    ...
    df["is_sus_aircraft"] = np.where(
        np.isin(df["AIRCRAFT_TYPE"], sus_air_type), 1, 0
    )

    if data_type == "train":
        df["UPGRADED_FLAG"] = df["UPGRADED_FLAG"].map({"Y": 1, "N": 0})

    df["is_sus_payment"] = np.where(
        np.isin(df["FORM_OF_PAYMENT"], sus_payment), 1, 0
    )
    df["intinerary_len"] = df["INTINERARY"].apply(lambda x: len(x.split("-")))
    df["is_sus_currency"] = np.where(
        np.isin(df["FORM_OF_PAYMENT"], sus_currency), 1, 0
    )

    df["PAX_GENDER"] = df["PAX_GENDER"].map({"M": 1, "F": 0})

    df["CORPORATE_CONTRACT_FLG"] = df["CORPORATE_CONTRACT_FLG"].map(
        {"Y": 1, "N": 0}
    )
    df["LOYAL_CUSTOMER"] = df["LOYAL_CUSTOMER"].map({"Y": 1, "N": 0})

    df["BOOKING_LONG_HOUL_FLAG"] = df["BOOKING_LONG_HOUL_FLAG"].map(
        {"Y": 1, "N": 0}
    )
    df["BOOKING_DOMESTIC_FLAG"] = df["BOOKING_DOMESTIC_FLAG"].map(
        {"Y": 1, "N": 0}
    )
...
def data_pipeline(data_path, model_type, data_type):
    df = read_all_csv(
        data_path=data_path, data_type=data_type, model_type=model_type
    )
    clean_data = clean_df(
        data=df,
        data_path=data_path,
